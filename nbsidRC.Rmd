...
```{r}
## A modifier
path_files <- "C:/Users/rmouster/Documents/perso/sfds/ouessant_copy/data" 
```
...
```
Nous remarquons que il y a beaucoup de valeur manquante pour la variable neige. Nous supposons que cette variable n'a pas été remplie quand il na pas neigé.
De plus nous remarquons aussi que il manque certaine données pour les variables nebul et rr_3h. Il faudra remplacer ou supprimer les valeurs manquantes.
...
```
Il semble y avoir un phénomène longitudinal, en effet la consommation journalière est plus basse en été que en hiver.
```{r}
...
Les valeurs manquantes ont été remplacées par la moyenne par mois et par heure (toutes les 3heures, donc), ce afin de risquer de biaiser l'étude. En effet, la température n'est pas la même le 1er janvier à midi qu'à minuit, et encore moins que le 1er juillet, par exemple.
...
Pour obtenir la meileure prédiction:
...
Le MAPE minimum obtenu est de **7.860093**
